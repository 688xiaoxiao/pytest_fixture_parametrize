...
@pytest.mark.parametrize('create_ios_driver_fix', [{'appium:app': '/path/to/app/UICatalog.app.zip'}], indirect=True)
...
@pytest.mark.parametrize('create_ios_driver_fix', [None], indirect=True)
def test_ios_click_without_options(appium_service, create_ios_driver_fix):
    with create_ios_driver_fix as driver:
        el = driver.find_element(by=AppiumBy.ACCESSIBILITY_ID, value='item')
        el.click()
